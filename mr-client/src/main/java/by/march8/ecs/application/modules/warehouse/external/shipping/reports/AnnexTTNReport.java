 ...
 */
public class AnnexTTNReport extends AbstractInvoiceReport {


    public AnnexTTNReport(SaleDocumentReport report) {
        super(report);
...
//            System.out.println(df.format(saleDocumentBase.getDocumentSaleDate().getTime()));
//            System.out.println(df.format(saleDocumentBase.getDocumentDate().getTime()));
//            //System.out.println(df.format(saleDocumentBase.getRefundDocDate().getTime()));
//            //System.out.println(df.format(saleDocumentBase.getEditRecordDate().getTime()));
//            System.out.println(df.format(saleDocumentBase.getCreateRecordDate().getTime()));
//            //System.out.println(df.format(saleDocumentBase.getSourceDocDate().getTime()));
//            System.out.println("_____________________________________________________");
            ...
            for (SaleDocumentDetailItemReport item : detailList) {
                ...
                if (saleDocumentBase.getDocumentNumber().trim().equals("ню7371284")
                        || saleDocumentBase.getDocumentNumber().trim().equals("ню7371737")) {
                    xCell = xSpreadsheet.getCellByPosition(5, row);
                    //xCell.setFormula(hm.get("cena").toString());
                    xCell.setFormula(String.valueOf(item.getValueCost()));

                    xCell = xSpreadsheet.getCellByPosition(6, row);
                    //xCell.setFormula(hm.get("summa").toString());
                    double ValueSumCost = round(item.getValueSumCost() / 100, ROUND_XXX_XX);
                    xCell.setFormula(String.valueOf(ValueSumCost));
                    ...
                    double ValueSumVat = round(ValueSumCost * (item.getValueVAT() / 100), ROUND_XXX_XX);
                    xCell.setFormula(String.valueOf(ValueSumVat));

                    xCell = xSpreadsheet.getCellByPosition(9, row);
                    //xCell.setFormula(hm.get("itogo").toString());
                    double ValueSumCostAndVat = round(ValueSumVat + ValueSumCost, ROUND_XXX_XX);
                    xCell.setFormula(String.valueOf(ValueSumCostAndVat));
                } else {
                    xCell = xSpreadsheet.getCellByPosition(5, row);
                    //xCell.setFormula(hm.get("cena").toString());
                    xCell.setFormula(String.valueOf(item.getValueCost()));
                ...
                xCell = xSpreadsheet.getCellByPosition(12, row);
                xCell.setFormula(item.getEanCode().trim() + "\nЦена произв.: " + String.valueOf(item.getValueCost()) + "\nопт. надбавка:0%\nопт. скидка: 0%");
