...
public class ProtocolStandardCurrencyReport extends AbstractInvoiceReport {

    public ProtocolStandardCurrencyReport(final SaleDocumentReport report) {
        super(report);
    ...
    boolean populateData(final XComponent component) {
        ...
        try {
            ...
            for (SaleDocumentDetailItemReport item : detailList) {
                ...
                if (documentReport.getDocument().getCurrencyId() == 2) {
                    Double price = (item.getAccountingPrice() * 100) / documentReport.getDocument().getCurrencyRateFixed();
                    xCell.setValue(Math.round(price * Math.pow(10, 2)) / Math.pow(10, 2));
                } else {
