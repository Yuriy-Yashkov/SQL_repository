 ...
 */
public class ProtocolBelPostReport extends AbstractInvoiceReport {
    public ProtocolBelPostReport(final SaleDocumentReport report) {
        super(report);
    ...
    ReportProperties prepareProperties(final ReportProperties properties) {
        ...
        properties.getTemplate().setTemplateName("протоколБелпочта20112018.ots");
        return properties;
    }
    ...
    boolean populateData(final XComponent component) {
        try {
            ...
            XCell xCell = xSpreadsheet.getCellByPosition(14, 1);
            ...
            for (SaleDocumentDetailItemReport item : detailList) {
                ...
                xCell.setFormula(item.getModelNumberAsString());
                ...
                //XCellRange xCellRange = xSpreadsheet.getCellRangeByPosition(0, row - 1, 0,row - 1);
                //setRowHeight(xCellRange, 1600);
                rowFormat(xSpreadsheet, row - 1, 15, CHAR_SIZE);
            }
            ...
            xCell.setFormula("\"" + getDataAsString("RECIPIENT_NAME") + "\": _________________/__________________");
            xPropSet = UnoRuntime
                    .queryInterface(XPropertySet.class,
                            xCell);
            xPropSet.setPropertyValue("HoriJustify", CellHoriJustify.LEFT);
