 ...
 */
public class ProtocolDororsReport extends AbstractInvoiceReport {
    public ProtocolDororsReport(final SaleDocumentReport report) {
        super(report);
    ...
    ReportProperties prepareProperties(final ReportProperties properties) {
        ...
        properties.getTemplate().setTemplateName("протоколДорорс.ots");
        return properties;
    }
    ...
    boolean populateData(final XComponent component) {
        try {
            ...
            XCell xCell = xSpreadsheet.getCellByPosition(9, 1);
            xCell.setFormula("№ " + getDataAsString("CONTRACT_NUMBER") + " от " + getDataAsString("CONTRACT_DATE_BEGIN") + "г.");

            String title = "между " + getDataAsString("RECIPIENT_NAME") + " и " + getDataAsString("SENDER_NAME");

            xCell = xSpreadsheet.getCellByPosition(0, 3);
            xCell.setFormula(title);
/*
            ...
            for (SaleDocumentDetailItemReport item : detailList) {
                ...
                if (retail != null) {
                    ...
                    BigDecimal bd = new BigDecimal(item.getValuePrice() * item.getValueVAT() / 100 + item.getValuePrice());

                    xCell.setValue(bd.setScale(10, BigDecimal.ROUND_HALF_UP).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue());
                ...
                rowFormat(xSpreadsheet, row - 1, 11, CHAR_SIZE);
            }

            row += 3;
            XPropertySet xPropSet;

            mergeCellRange(xSpreadsheet, 1, row, 6, row);
            ...
            mergeCellRange(xSpreadsheet, 7, row, 10, row);
            xCell = xSpreadsheet.getCellByPosition(7, row);
            ...
            mergeCellRange(xSpreadsheet, 7, row, 10, row);
            xCell = xSpreadsheet.getCellByPosition(7, row);
            xCell.setFormula("______________________________ / ___________");
            xPropSet = UnoRuntime
                    .queryInterface(XPropertySet.class,
                            xCell);
            xPropSet.setPropertyValue("HoriJustify", CellHoriJustify.LEFT);
