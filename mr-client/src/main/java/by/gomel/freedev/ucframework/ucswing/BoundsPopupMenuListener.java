...
public class BoundsPopupMenuListener implements PopupMenuListener {
     ...
     */
    public BoundsPopupMenuListener(final boolean popupWider,
                                   final boolean popupAbove) {
     ...
     *            popup is greater than width of scrollPane.
     * @param popupWider
     *            display the popup at its preferred with
     * @param maximumWidth
     *            limit the popup width to the value specified (minimum size
     *            will be the width of the combo box)
     * @param popupAbove
     *            display the popup above the combo box
     *
     */
    public BoundsPopupMenuListener(final boolean scrollBarRequired,
                                   final boolean popupWider, final int maximumWidth,
                                   final boolean popupAbove) {
     ...
     *
     * @param maximumWidth
     *            the maximum width of the popup. The popupAbove value is set to
     *            "true".
     */
    public BoundsPopupMenuListener(final int maximumWidth) {
     ...
     *
     * @param popupAbove
     *            true display popup above the combo box, false display popup
     *            below the combo box.
     */
    public void setPopupAbove(final boolean popupAbove) {
     ...
     */
    public void setPopupWider(final boolean popupWider) {
     ...
     *
     * @param scrollBarRequired
     *            true add horizontal scrollBar to scrollPane false remove the
     *            horizontal scrollBar
     */
    public void setScrollBarRequired(final boolean scrollBarRequired) {
    ...
    public void popupMenuWillBecomeVisible(final PopupMenuEvent e) {
        ...
        final Object child = comboBox.getAccessibleContext()
    ...
    protected void customizePopup(final BasicComboPopup popup) {
        ...
        if (popupAbove) {
            final int height = popup.getPreferredSize().height;
            popup.setLocation(location.x, location.y - height);
        } else {
            final int height = comboBox.getPreferredSize().height;
     ...
     */
    protected int getScrollBarWidth(final BasicComboPopup popup,
     ...
     */
    protected JScrollPane getScrollPane(final BasicComboPopup popup) {
        final JList list = popup.getList();
        final Container c = SwingUtilities.getAncestorOfClass(
    ...
    protected boolean horizontalScrollBarWillBeVisible(
            final BasicComboPopup popup, final JScrollPane scrollPane) {
        final JList list = popup.getList();
        final int scrollBarWidth = getScrollBarWidth(popup, scrollPane);
        final int popupWidth = list.getPreferredSize().width + scrollBarWidth;
     ...
     */
    protected void popupWider(final BasicComboPopup popup) {
        final JList list = popup.getList();
        ...
        int popupWidth = list.getPreferredSize().width + 5 // make sure
                // horizontal
                // scrollbar doesn't
                // appear
    ...
    /*
     * This method is called every time: - to make sure the viewport is returned
     * to its default position - to remove the horizontal scrollbar when it is
     * not wanted
     */
    private void checkHorizontalScrollBar(final BasicComboPopup popup) {
