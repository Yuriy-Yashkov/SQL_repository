 ...
 * @date 15.12.2011
 */
public class ProductionOO extends OO_new {
    //private static final Logger log = new Log().getLoger(ProductionOO.class);
     ...
     */
    public ProductionOO(String nameReport, Vector v, Vector dopv) {
        vec = v;
        dopVec = dopv;
    ...
    public void fildU4otIP(XComponent currentDocument) throws Exception {
        try {
...
//----------------Заполнение тела документа
            ...
            for (Object item : vec) {
                for (int i = 0; i < 9; i++) {
                    ...
                    if (((Vector) item).get(i) != null) {
                        xCell.setFormula(((Vector) item).get(i).toString());
                    } else xCell.setFormula("0");
                    xCell = xSpreadsheet.getCellByPosition(i, nRow);
            ...
            log.error("Ошибка при заполнении учёта инновационной продукции ", e);
            throw new Exception("Ошибка при заполнении отчёта учёта инновационной продукции ", e);
        }
    }
    ...
    public void createReport(String nameTamplates) throws Exception {
        try {
            ...
            if (nameTamplates.equals("УчётИнновационнойПродукции.ots")) {
                fildU4otIP(currentDocument);
            }
        } catch (java.lang.Exception e) {
            System.out.println(e.getMessage());
