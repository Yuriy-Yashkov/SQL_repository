...
@NamedQueries({
        @NamedQuery(name = "RemainPriceListDetailItem.findByPriceList",
                query = "SELECT item FROM RemainPriceListDetailItem item WHERE item.priceLilstId = :priceList " +
                        "order by " +
                        " item.name ")
})

@Entity
@Table(name="V_REMAINS_PRICELIST_DETAIL")
@RedirectToEntityManager(redirectTo = MarchDataSourceEnum.DS_SQLMARCH8)
public class RemainPriceListDetailItem extends BaseEntity {
